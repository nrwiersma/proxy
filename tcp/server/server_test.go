...
func newTestServer(t testing.TB, h Handler, opts Opts) (net.Addr, *Server) {
...
func TestServer_ServesConnectionCloses(t *testing.T) {
	addr, srv := newTestServer(t, pingHandler{close: true}, Opts{
...
func TestServer_ServesConnectionStaysOpen(t *testing.T) {
	addr, srv := newTestServer(t, pingHandler{}, Opts{
		ReadTimeout:  time.Second,
		WriteTimeout: time.Second,
		IdleTimeout:  time.Second,
	})
...
func (h pingHandler) ServeTCP(ctx context.Context, conn Conn) {
	var ping [4]byte
